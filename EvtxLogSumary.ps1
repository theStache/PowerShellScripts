...
#Reads in a Windows Event Log, selects column containing Windows Event ID information; provides a new column with the event Id details
#Change the import-csv/export-csv options to #desired locations.
#Use at own risk; Tyler Williams - tjwill86@gmail.com; 20181204
#Work in progress, this script is rough and needs some polishing
...
$item = [int]$item
$element = $eventtable[$item]
return "$element"
}

echo "Staring time :"; date
#Reading in an event log
$log = Get-WinEvent -Path "\\Path\To\Security_Logs\Security.evtx"
#Read EXTX log getting the EID and counting by type and sorting them highest number to lowest
$newcsv = $log | Group-object id -NoElement | sort count -descending | select Name, Count
#attempt to free space
$log = $null
#Create two new csv columns
$newcsv = ($newcsv | Select-Object *,@{Name="ExpandedEventID";Expression='.'},@{Name="Percentage";Expression='.'})
#Pass event Id names to get the expanded name added to the newly created column
$newcsv| %{$_.ExpandedEventID = dataParser($_.Name) };
#Initiate a variable
$sum = 0
#Get total number of EIDs
$newcsv | ForEach-Object{$sum += $_.Count}
#Get percentage of EID
$percentages = $newcsv | foreach{$x=$_.Count / $sum * 100; echo $x}
for ($x=0; $x -lt $percentages.count; $x++){
    $newcsv[$x].Percentage = $percentages[$x]
    }
#attempt to free space
$percentages = $null
#Exporting the result to CSV
$newcsv | Export-Csv -NoTypeInformation '\\Path\To\Output.csv'
